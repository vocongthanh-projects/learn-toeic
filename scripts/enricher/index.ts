...
function getDefaultNodeForPart(part: number): string {
  switch (part) {
    case 1: return 'listening.part1.photo_description';
    case 2: return 'listening.part2.question_response';
    case 3: return 'listening.part3.conversation';
    case 4: return 'listening.part4.talk';
...
function assignKnowledgeNodes(q: Question): string[] {
  // Listening (Part 1-4) has no grammar-keyword taxonomy of its own yet — don't run the
  // Part 5 grammar-keyword cascade against a spoken transcript, it would mislabel the topic.
  if (q.part <= 4) return [];
